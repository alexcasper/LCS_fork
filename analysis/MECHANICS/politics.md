...
The game tracks 22 laws (`LAWNUM`), each rated on the alignment scale (-2 to +2):
...
Two additional derived indicators — **Mood** and **Stalin** — track overall political climate and authoritarian-left drift. These are not stored in `law[]` but are calculated from the other law values.
